function [outputArg1,outputArg2] = export_btn_ml(hObject,handles, modelType, plotName, DATA)
...
%     end
  ...
  [FileName,PathName] = uiputfile({'*.csv','*.xlsx'})  
   writetable(DATA, fname);
         
  else % 2014 or older   
  % find another solution for data export      
  end
