...
if ~ischar( label )
    ...
    if (~ischar( label ))||(~isnumeric(level))
...
if entry_added
    ...
    entries = iAddEntry( figh, entries, label, 0, defbarcolor, bgcol,level);
    idx = numel( entries );
end
...
uicontrol(f,'Style', 'pushbutton', 'String', 'Cancel (or MainWin:Ctrl+C)',...
        'Position', [10 7 150 20],...
        'Callback', @cancelOperations);  
...
%-------------------------------------------------------------------------%
function entries = iAddEntry( parent, entries, label, value, color, bgcolor,level)
% Add a new entry to the progress bar

% Create bar coloring
psize = getappdata( parent, 'DefaultProgressBarSize' );
psize(1) = psize(1) - level * 25;
...
set( entry.LabelText, 'String', [entry.Label, entry.addText] );

if(entry.ShowTime)

if elapsedtime > minTime
    if force || (decval ~= lastdecval)
        labelstr = [entry.Label, entry.addText, sprintf( ' (%d%%)', decval )];
...
requiredheight = (2*border + N*heightperentry - panelheight) + 35;
...
for ii=1:numel( entries )
    set( entries(ii).LabelText, 'Position', [border+entries(ii).level*25 ypos-textheight width*0.75-entries(ii).level*25 textheight] );
    ...
    if entries(ii).CanCancel
        set( entries(ii).Progress, 'Position', [border+entries(ii).level*25 ypos-barheight width-barheight+1-entries(ii).level*25 barheight] );
        entries(ii).ProgressSize = [width-barheight-entries(ii).level*25 barheight];
        set( entries(ii).CancelButton, 'Visible', 'on', 'Position', [p(3)-border-barheight ypos-barheight barheight barheight] );
    else
        set( entries(ii).Progress, 'Position', [border+entries(ii).level*25 ypos-barheight width+1-entries(ii).level*25 barheight] );
        entries(ii).ProgressSize = [width-entries(ii).level*25 barheight];
